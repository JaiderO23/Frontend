...
export default function Ventas() {
  ...
  const ventasFiltradas = ventas.filter(v => {
    ...
    const fechaVenta = new Date(v.creadoEn).toISOString().split('T')[0]
  ...
  const columns = [
    ...
    },
    {
      key: 'cliente',
      label: 'Cliente',
      render: (value) => value ? `${value.nombre} ${value.apellido}` : 'Cliente General'
    },
    {
      key: 'tipoVenta',
      label: 'Tipo',
  ...
  const ventasContado = ventasFiltradas.filter(v => v.tipoVenta === 'CONTADO').length
  const ventasCredito = ventasFiltradas.filter(v => v.tipoVenta === 'CREDITO').length
  
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          <ShoppingCart className="animate-pulse mx-auto mb-4 text-blue-600" size={48} />
        ...
        <Card>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">{ventasContado}</p>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">{ventasCredito}</p>
            </div>
          </div>
        </Card>
      </div>
